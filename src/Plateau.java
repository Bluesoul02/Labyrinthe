...
class Plateau {
    ...
    protected void setCouloirFixe() {
        ...
        for (int i = 0; i < 4; i++) {
            couloirs.add(new CouloirFixe(orients[i], Forme.TE, objectifs[i], positions[i]));
        }

        int[][] values = { { 0, 2, 0, 4 }, { 2, 0, 4, 0 }, { 6, 2, 6, 4 }, { 2, 6, 4, 6 } };
        for (int i = 0; i < 4; i++) {
            couloirs.add(new CouloirFixe(orients[i], Forme.TE, objectifs[i], new Position(values[i][0], values[i][1])));
            couloirs.add(new CouloirFixe(orients[i], Forme.TE, objectifs[i], new Position(values[i][2], values[i][3])));
        }
    }
